...
def drush_aliases(config, project_root)
  ...
  config.trigger.after [:up, :reload, :resume] do
    ...
    system "drush #{project_root}/.valkyrie/valkyrie/vagrant/lib/bin/set_alias_path.php #{aliases_path}"
  end

  config.trigger.after [:destroy] do
    # Remove drush aliases
    system "drush #{project_root}/.valkyrie/valkyrie/vagrant/lib/bin/unset_alias_path.php #{aliases_path}"
  end
